...
func (d *SysRoleData) Paging(ctx context.Context, req *options.SysRolePagingOptions) (list []*types.SysRole, p *shared.Paginated, err error) {
	query := bson.M{}
	cursor, p, err := d.mgo.Paginate(ctx, entity.TableSysRole, query, req.Paginate)
	if err != nil {
		return
	}
